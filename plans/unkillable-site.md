...
**PRD:** https://github.com/stillmusic-tech/unkillable/issues/1 (vault twin: `PRDs/Unkillable/Unkillable — Site PRD.md`)  ·  **Status:** Phases 1–14 COMPLETE. All seven attacks playable; report card + share done (built onto the quantum page, the site finale per the register). Phases 15–17 done: fundamentals ladder + overlay, About page + enforced sourcing-sweep audit test, and the phone-viewport touch pass (11 pages no-overflow + tap interactions, all green). Every AFK phase (1–17) COMPLETE and **DEPLOYED LIVE** (2026-08-09) to unkillable.bitcoin-fix.com — all 11 pages verified live. 148 tests green (108 Vitest + 40 Playwright). Only the merged Phase 3+18 HITL full-site feel review remains: Laurence plays the whole site, punch list gathered and fixed.

**Phase 11 rendering decision (the wall spike):** rendering 900k+ blocks smoothly is solved by **canvas virtualization** — never touch all blocks; from scroll position + zoom compute only the bounded on-screen window (`src/engines/wall/engine.ts`, `visibleWindow`/`renderColumns`). Zoomed in: one rect per block; zoomed out: one column per pixel aggregating many blocks. Draw cost is O(viewport), not O(chain) — proven by a test asserting the drawn-column count stays ~viewport-bounded even at 10× the current tip. No DOM-per-block, no library. This is what Attack 6 ships on.
 Building all remaining phases before one merged HITL feel review (Laurence's call, 2026-08-09: build everything, then test). **Phase 4** (2026-08-09) — the data tap (`src/tap/tap.ts`, live mempool.space + bundled fallback, mode + date exposed) and the live homepage block strip with ~10-min countdown. **Phase 5** (2026-08-09) — the observatory globe (`src/globe/globe.ts`, shared with Attack 2) with real node coordinates, drag-to-spin, pulsing arcs. 45 tests green (29 Vitest + 16 Playwright). Phase 3 (Attack 1 feel check) folds into the final Phase 18 full-site review. Next: Phase 6 (the armoury).
